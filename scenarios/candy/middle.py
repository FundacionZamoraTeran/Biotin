...
from scenarios.candy import portal
...
class Middle:
    """
        Class representing the second part of the candy woods, recieves
                ...
                if event.type == pygame.KEYDOWN:
                    if event.key == pygame.K_LEFT or event.key == pygame.K_KP4:
                        ...
                        if (self.player.real_x+self.player.rect.width > 2280 and
                            ...
                            por = portal.Portal(self.screen, self.clock, self.character)
                            por.run()
                            del por
                            running = False
    ...
    def actors_load(self, rel_x):
        if self.player.real_x < 1500 and not self.gali.defeated:
            ...
            if self.gali.alive():
                self.gali.remove(self.enemies_list)
        if self.player.real_x < 2401 and not self.menti.defeated:
            ...
            if not self.menti.transformed:
                ...
                self.screen.blit(self.menti.sprites["transform"], (self.menti.x-rel_x, 620))
        else:
            if self.menti.alive():
                self.menti.remove(self.enemies_list)
        if self.player.real_x > 800 and not self.vlopop.defeated:
